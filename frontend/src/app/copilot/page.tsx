...
export default function CopilotPage() {
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hello! I am SentinelNexus Executive Copilot. How can I help you analyze your security posture today?" }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      // The backend expects the list of previous messages to maintain context
      ...
      <div className="space-y-6 pb-8 h-full flex flex-col">
        ...
        >
          ...
          <motion.button 
            ...
            onClick={() => setMessages([{ role: "assistant", content: "Chat history cleared. How can I assist you?" }])}
          >
            Clear Chat
          </motion.button>
        </motion.div>
          ...
          <div className="flex-1 flex flex-col gap-4 overflow-y-auto pb-20 scrollbar-hide">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-blue-500/20 text-blue-400' : 'bg-violet-500/20 text-violet-400'}`}>
                  {msg.role === 'user' ? '👤' : '🤖'}
                </div>
                ...
                } max-w-[80%] prose prose-invert prose-p:leading-relaxed prose-pre:bg-black/50 prose-pre:border prose-pre:border-white/10`}>
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <div className="relative">
              <input 
                ...
                placeholder="Ask SentinelNexus a question..." 
                className="w-full bg-black/60 backdrop-blur-xl border border-white/[0.1] rounded-xl px-4 py-4 text-sm text-white outline-none focus:border-violet-500 pr-12 shadow-2xl transition-all" 
              />
              <SpecularButton 
                onClick={sendMessage}
                className="absolute right-2 top-2 bottom-2 w-10 rounded-lg bg-violet-500/20 hover:bg-violet-500 flex items-center justify-center text-violet-300 hover:text-white transition-all"
              >
                 ↑ 
              </SpecularButton>
            </div>
